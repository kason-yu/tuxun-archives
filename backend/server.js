...
app.get('/api/countries/:country', async (req, res) => {
  ...
  const { data, error } = await supabase.from('countries').select('*').eq('name', country);
...
app.put('/api/countries/:country/:column', async (req, res) => {
  ...
  // 查找或创建国家记录
  let { data: existing } = await supabase.from('countries').select('*').eq('name', country);
  
  if (!existing || existing.length === 0) {
    // 创建新国家
    const { error } = await supabase.from('countries').insert([{
      name: country,
      data: { [column]: Number(value) || 0 }
    }]);
    if (error) return res.status(500).json({ error: error.message });
  } else {
    // 更新现有国家
    const newData = { ...existing[0].data, [column]: Number(value) || 0 };
...
app.delete('/api/countries/:country', async (req, res) => {
  const { country } = req.params;
  const { error } = await supabase.from('countries').delete().eq('name', country);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ success: true });
});
